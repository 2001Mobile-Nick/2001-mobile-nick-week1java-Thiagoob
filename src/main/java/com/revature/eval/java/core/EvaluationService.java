...
public class EvaluationService {
    ...
    public int getScrabbleScore(String string) {
    	...
    	String lettersPointBased[] = {"AEIOULNRST", "DG", "BCMP", "FHVWY", "K"};
    	
    	for (int location = 0; location < lettersPointBased.length; location++)
    	{
    		int length = lettersPointBased[location].length();
    		for (int index = 0; index < length; index++)
    		{
    			letterScores.put(lettersPointBased[location].charAt(index), location + 1);
    		}
    	}
    ...
    public Temporal getGigasecondDate(Temporal given) {
    	...
    	long seconds = (long) Math.pow(10, 9);
    	
    	Temporal start = null;
    	
    	if (given instanceof LocalDate)
    	{
	    	start = (LocalDate) given;
	    	
	    	start = ((LocalDate)start).atStartOfDay();
    	}
    	...
    	LocalDateTime date = ((LocalDateTime)start).plusSeconds(seconds);
